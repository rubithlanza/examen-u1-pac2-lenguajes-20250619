...
export const RolesPage = () => {
  ...
  const handlePreviousPage = () => {
    if (rolePaginationQuery.data?.data.hasPreviousPage) {
      setPage(page - 1)
    }
  }

  const handleNextPage = () => {
    if (rolePaginationQuery.data?.data.hasNextPage) {
      setPage(page + 1)
    }
  }
    ...
    <div className="w-full flex flex-col">
      ...
      <div className="rounded-md mt-4 overflow-hidden">
        <table className="w-full">
          ...
          <tbody>
            {rolePaginationQuery.data &&
              rolePaginationQuery.data.data.items.map(role => (
        ...
        <div className="flex flex-row items-center justify-between bg-blue-500">
          <span className="p-2 text-white">Registros {rolePaginationQuery.data?.data.totalItems}</span>
          <div className="flex flex-row items-center gap-2">
            <button
              onClick={handlePreviousPage}
              disabled={!rolePaginationQuery.data?.data.hasPreviousPage}
              className={`flex flex-row items-center justify-center gap-2 p-2 ${
                !rolePaginationQuery.data?.data.hasPreviousPage
            ...
            <button
              ...
              className={`flex flex-row items-center justify-center gap-2 p-2 ${
                !rolePaginationQuery.data?.data.hasNextPage
                ? "bg-blue-500 text-white rounded-md cursor-not-allowed" 
                : "bg-blue-500 text-white hover:bg-blue-600 rounded-md cursor-pointer"
              }`} >
              <span>Siguiente</span>
